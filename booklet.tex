...
\renewcommand{\chaptername}{Area of Study}}
...
\renewcommand\contentsname{Areas of Study}
\tableofcontents

\chapter*{Questions}

\section*{Area of Study 1}
\begin{enumerate}
...
\begin{enumerate}
	...
	\end{enumerate}
	\item The people of the states and territories vote YES or NO to the proposal. In order for the referendum to pass, it must receive a \emph{double majority}. That is to say, it must be supported by a majority of the states and by a majority of the people.
	\begin{enumerate}
...
Alternative Dispute Resolution (ADR) methods, as the name suggests, provide an alternative to dispute resolution through judicial determination. These methods are often called `primary dispute resolution', suggesting that some civil disputes ought to be settled in this way initially.

\subsection{Types of ADR}
...
\subsection{Strengths of ADR}

\begin{description}
	\item[`Voluntary'] Alternative dispute resolution is usually seen as a voluntary process compared to a formal court hearing, which could be described as compulsory dispute settlement.
	\item[Accessible] ADR is generally less formal than a normal court hearing. This makes the process less intimidating to the parties.
	\item[Low Cost] ADR is usually a cheaper alternative to a formal court hearing due to the parties often not requiring legal representation. In instances where legal representation is used, the fact that ADR can allow for faster decisions than a typical hearing helps to keep costs down.
	\item[Timely] Strict rules of evidence and procedure do not apply in ADR which can allow for a decision to be made more speedily compared with a formal court hearing.
	\item[Confidential] Unlike formal court hearings, which are normally open to the public, ADR can be conducted in private.
	\item[Flexible] In a court hearing, the judge or magistrate (and jury if relevant) hears the evidence put forward by each party and then makes a decision which is binding on both parties with a `winner' and a `loser'. Whereas through ADR, if the parties reach a settlement, an agreement can be drawn up that incorporates the terms of the settlement and this would become a binding contract. However, a party cannot be forced to consent to an agreement with which he/she is not happy. 
\end{description}

\subsection{Weaknesses of ADR}

\begin{description}
	\item[Unwillingness to Co-operate] One party might not wish to resolve the conflict and might use ADR to waste time in order delay a resolution of the conflict, or to increase the expense for the other party.
	\item[Potential Power Imbalance] Mediation is less likely to be successful where there is a power imbalance between the parties, or in marital disputes where there has been violence.
	\item[Not Binding] A mediated agreement is not binding and enforceable by the courts unless the agreement has been incorporated into a court order, or amounts to a new contractual agreement.
	\item[Potentially Expensive] Arbitration can still be quite expensive and adversarial (considering that many parties still use legal representation).      
\end{description}
...
However, sometimes the element is not achieved, such as judges often affected by personal biases or not being representative of society, particularly in relation to indigenous parties. One recommendation to improve this is to require all judges to attend compulsory training in relation to the Aboriginal culture. This will ensure they are more familiar with the Aboriginal culture and are more sensitive to particular issues, thus ensuring indigenous people have access to a fair and unbiased hearing
...
\subsection{Strengths of the Adversary System}
...
\subsection{Weaknesses of the Adversary System}
...
\begin{description}
	\item[Inquisitorial System] One possible improvement could be to incorporate elements of the inquisitorial system. This could include a greater investigative role of the judge may lead to a greater emphasis on ascertaining the truth.
	\item[ADR] A greater use of alternate methods of dispute resolution could be encouraged in civil cases. For instance, by requiring parties to enter mediation before a matter can go to court.
	\item[Legal Aid] Provide greater access to legal aid.
	\item[Reform the Court Procedure] A less formal conduct of the trial and an absence of strict rules of evidence and procedure means that witnesses may give evidence in narrative form. This could be prevent useful evidence from being excluded since the witness tells all relevant parts of the story. Counsel can still question witnesses.
\end{description}
...
\begin{description}
	\item[Burden of proof] the responsibility is placed on the party bringing the case to court to prove the case against the other party. In criminal cases the burden lies with the prosecution who must prove that the accused is guilty. This links with the principle of innocent until proven guilty. 
	\item[Standard of proof] the extent to which the party bringing the case to court must prove the case. In criminal trials the standard of proof is `beyond reasonable doubt'.
\end{description}
